  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      <motion.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div><h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t('performance')}</h1><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('managePerformanceDesc')}</p></div>
        <Button size="sm" icon={Target}>{t('newReviewCycle')}</Button>
      </motion.div>
      ...
      <motion.div variants={fadeUp} className="rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-12">
        <div className="text-center">
          ...
          <h3 className="mt-4 text-sm font-semibold text-gray-900 dark:text-gray-100">{t('performanceComingSoon')}</h3>
