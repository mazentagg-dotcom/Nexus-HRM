    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={fadeUp}>
        <AnimatedTable columns={[
          ...
          { key: 'leave_type', label: 'Type', render: v => <Badge color={typeColors[v] || 'gray'}>{v}</Badge> },
      ...
      <Modal isOpen={showLeaveModal} onClose={resetForm} title="New Leave Request" size="md" footer={
        <><Button variant="secondary" onClick={resetForm}>Cancel</Button><Button loading={submitting} onClick={() => { const f = document.getElementById('leave-form'); if (!f) return; const data = Object.fromEntries(new FormData(f)); const start = new Date(data.start_date); const end = new Date(data.end_date); data.duration_days = Math.max(1, Math.round((end - start) / (1000 * 60 * 60 * 24)) + 1); setSubmitting(true); createLeaveRequest(data).then(() => { showToast('Leave request submitted', 'success'); resetForm() }).catch(e => { showToast(e.response?.data?.message || 'Error', 'error'); setSubmitting(false) }) }}>Submit</Button></>
      }>
        <form id="leave-form" className="space-y-4">
          ...
          <Select label="Leave Type" name="leave_type" options={[{ value: 'annual', label: 'Annual Leave' }, { value: 'sick', label: 'Sick Leave' }, { value: 'personal', label: 'Personal Leave' }, { value: 'unpaid', label: 'Unpaid Leave' }]} placeholder="Select type" required />
