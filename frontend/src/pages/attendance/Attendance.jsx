    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="card-base p-5 flex items-center gap-4">
          ...
          <div><p className="text-sm text-gray-500">Present</p><p className="text-2xl font-bold text-gray-900">{loading ? '-' : present}</p></div>
        </div>
        <div className="card-base p-5 flex items-center gap-4">
          ...
          <div><p className="text-sm text-gray-500">Late</p><p className="text-2xl font-bold text-gray-900">{loading ? '-' : late}</p></div>
        </div>
        <div className="card-base p-5 flex items-center gap-4">
          ...
          <div><p className="text-sm text-gray-500">Absent</p><p className="text-2xl font-bold text-gray-900">{loading ? '-' : absent}</p></div>
        </div>
      </motion.div>
      ...
      <motion.div variants={fadeUp}>
        <AnimatedTable columns={[
          ...
          { key: 'date', label: 'Date' },
          { key: 'check_in', label: 'Check In', render: v => <span className="text-gray-500 text-xs">{v || '--'}</span> },
          { key: 'check_out', label: 'Check Out', render: v => <span className="text-gray-500 text-xs">{v || '--'}</span> },
          { key: 'work_hours', label: 'Hours', render: v => <span className="text-gray-500 text-xs">{v || '--'}</span> },
